...
** fonts
 #+BEGIN_SRC emacs-lisp
   ...
   (defun hesam/font-monitors ()
     (if window-system			; check X-window
         (progn
           (dolist (elem (frame-list))	; for all frames
             (if (> (car (last (assq 'geometry (frame-monitor-attributes elem)))) 2000)
                 (set-frame-parameter elem 'font "Source Code Pro 15")
               (set-frame-parameter elem 'font "Source Code Pro 10"))))))
   ...
   ;; TODO: loop over display-attributes, check if frame is in frames list. If so,
   ;; check dim of monitor and set font accordingly. Use
   ;; (push 'hesam/font-per-display after-make-frame-functions)
   ;; to run the function every time a new frame is connected
...
#+BEGIN_SRC emacs-lisp
  (defun my-nov-font-setup ()
    (face-remap-add-relative 'variable-pitch :family "Source Sans Pro Light"
                             :height 220))
